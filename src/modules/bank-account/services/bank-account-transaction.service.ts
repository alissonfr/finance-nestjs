...
export class BankAccountTransactionService {
    ...
    async find({ month, year, search, sort }): Promise<BankAccountTransactionResponseDTO[]> {
        ...
        const mappedResults = await Promise.all(
            results.map(async transaction => {
                ...
                const recurrency = {
                    installment:
                        installments.findIndex(t => t.bankAccountTransactionId === transaction.bankAccountTransactionId) + 1,
    ...
    async create(input: BankAccountTransactionRequestDTO): Promise<void> {
        ...
        try {
            ...
            for (let i = 0; i < numberOfInstallments; i++) {
                ...
                const amount =
                    input.type === TransactionType.IN_INSTALLMENTS
                        ? accounting.toFixed(input.amount / input.installments, 2)
                        : input.amount
    ...
    async update(id: number, updateData: Partial<BankAccountTransaction>): Promise<BankAccountTransaction> {
